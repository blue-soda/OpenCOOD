...
from opencood.tools.codyntrust_matcher import Matcher
...
class PointPillarCodyntrustCobevflowWUncertainty(nn.Module):
    def __init__(self, args):
        super(PointPillarCodyntrustCobevflowWUncertainty, self).__init__()
        print("=== train CoDynTrust CoBEVFlow with uncertainty ===")
        # PIllar VFE
    ...
    def generate_box_flow(self, data_dict, pred_dict, dataset, shape_list, device, pairwise_t_matrix): 
        ...
        for b in range(B):
            ...
            predict_unc_cls = predict_unc_cls_list[b].reshape(-1, self.k, 4, predict_unc_cls_list[b].shape[-2], predict_unc_cls_list[b].shape[-1]) # (N_b, k, 4, H, W)
    ...
    def forward(self, data_dict, dataset=None):
        ...
        # Re-parameterization can regularize the classification logit variance.
        ...
        if self.inference_state is True:
            ...
            zero_unc = torch.zeros(
                psm_single.shape[0], psm_single.shape[1] * 2,
                psm_single.shape[2], psm_single.shape[3],
                device=psm_single.device,
                dtype=psm_single.dtype)
            predict_unc = {'predict_unc_cls': self.regroup(zero_unc, record_len, k),
                           'predict_unc_reg': self.regroup(zero_unc, record_len, k)}
